document.addEventListener("DOMContentLoaded", function () {
  ...
  function fetchTransactions() {
    ...
    axios
      .get(
        `${API_BASE_URL}/get_transactions.php?` + params.toString(),
        axiosConfig
      )
    ...
    transactions.forEach((tx) => {
      ...
      if (type === "transfer") {
        ...
        if (meta && meta.from_sym && meta.to_sym) {
          ...
          const ratePart =
            meta.rate != null ? ` @ ${Number(meta.rate).toFixed(8)}` : "";
          const usdtPart =
            meta.usdt_value != null
              ? ` (≈ $${Number(meta.usdt_value).toFixed(2)})`
              : "";
