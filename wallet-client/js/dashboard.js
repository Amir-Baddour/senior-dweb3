...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (userNameElem && userMetaElem) {
    axios
      .get('http://localhost/digital-wallet-plateform/wallet-server/user/v1/get_profile.php', axiosConfig)
      .then((response) => {
        if (response.data?.success) {
          ...
          userMetaElem.textContent = userTier ? `User Level: ${userTier}` : 'User Level: Regular';
        } else {
  ...
  if (verificationWidget && verificationTitle && verificationMessage && verificationButton) {
    axios
      .get('http://localhost/digital-wallet-plateform/wallet-server/user/v1/get_verification_status.php', axiosConfig)
      .then((response) => {
        ...
        switch (status) {
          ...
          case -1:
            ...
            verificationButton.onclick = () => (window.location.href = 'verification.html');
            break;
          default:
    ...
    async function fetchBalance() {
      ...
      try {
        ...
        if (map && Object.keys(map).length) {
          if (map.USDT !== undefined) return { amount: Number(map.USDT), symbol: 'USDT' };
  ...
  function updateProgressBar(used, limit, barElem, infoElem) {
    ...
    if (infoElem) infoElem.textContent = `${Number(used).toFixed(2)} / ${Number(limit).toFixed(2)}`;
  }

  if (dailyInfo && weeklyInfo && monthlyInfo && dailyBar && weeklyBar && monthlyBar) {
    axios
      .get('http://localhost/digital-wallet-plateform/wallet-server/user/v1/get_limits_usage.php', axiosConfig)
      .then((response) => {
        if (response.data?.error) {
          dailyInfo.textContent = weeklyInfo.textContent = monthlyInfo.textContent = 'Error';
        } else {
